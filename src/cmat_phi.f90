subroutine cmat_phi(pin,sp,ik)
    ...
    do ia=1,natom
      ...
      do is=1,ms
        ca(is) = sum(conjg(at(ia)%local_c1_c(:,:,:,is))*pin)*dv
      enddo

      if(.not.at(ia)%edge) then
        do is=1,ms
          sp(ixs:ixe,iys:iye,izs:ize)=sp(ixs:ixe,iys:iye,izs:ize)+&
            at(ia)%local_p3d1_c(:,:,:,is,1)*sum(at(ia)%cmat(is,:)*ca(:))
        enddo
      else
        do iz=izs,ize;do iy=iys,iye;do ix=ixs,ixe
          ...
          do is=1,ms
            sp(jx,jy,jz)=sp(jx,jy,jz)+at(ia)%local_p3d1_c(ix-ixs+1,iy-iys+1,iz-izs+1,is,1)&
                *sum(at(ia)%cmat(is,:)*ca(:))
          enddo
        enddo;enddo;enddo
      endif
      deallocate(ca)
    enddo
end subroutine cmat_phi

subroutine add_ctaylor_phi(pin,sp,ik)
    use main_mod
    use atom_mod
    use atom_mod, only : p=> pawinfo, at=> atominfo
    use mpi_lib_ours, only : rank
    ...
    do ia=1,natom
      ...
      if(.not.at(ia)%edge) then
        do is=1,ms
          sp(ixs:ixe,iys:iye,izs:ize)=sp(ixs:ixe,iys:iye,izs:ize)+&
            at(ia)%local_p3d1_c(:,:,:,is,1)*sum(at(ia)%c_taylor(is,:)*ca(:))
        enddo
      else
        do iz=izs,ize;do iy=iys,iye;do ix=ixs,ixe
          jx=mod(ix+nx-1,nx)+1
      ...
      deallocate(ca)
    enddo
end subroutine add_ctaylor_phi

subroutine add_ctaylor_phi1(pin,sp,ik)
    use main_mod
    use atom_mod
    ...
    do i=1,tbar_taylor_power
      ...
      do ia=1,natom
        ...
        do is=1,ms
          ca(is) = sum(conjg(at(ia)%local_c1_c(:,:,:,is))*tmp)*dv/dble(i)
        enddo
      ...
      tmp = tmp1
    enddo
end subroutine add_ctaylor_phi1

subroutine c_phi(pin,sp,ik)
    use main_mod
    use atom_mod
    use atom_mod, only : p=> pawinfo, at=> atominfo
    use mpi_lib_ours, only : rank
    ...
    do ia=1,natom
      ...
      do is=1,ms
        ca(is) = sum(conjg(at(ia)%local_c_c(:,:,:,is))*pin)*dv
      enddo
...
subroutine c_taylor_loop(pin,pout,ik)
  use main_mod
