...
!   write(*,*) 'sum dens', sum(abs(phit_tot(:,:,:,1:nocc,1)**2))*dv
   ...
   if(flg_bin) then
     if(rank==0) then
       ...
       open(unit=1,file='eig_debug')
       read(1,*) eig
       close(1)

       open(unit=1,file='wf_debug.bin',form='unformatted')
       write(1) 'nx      ',nx
       write(1) 'ny      ',ny
       write(1) 'nz      ',nz
       write(1) 'dx      ',dx
