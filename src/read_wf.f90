...
subroutine read_wf_tddft
  ...
  call check_r(dz,dz_r,' dz, dz_r')
  call check(1,nsp_r,' nsp, nsp_r')
  !if (tddft_flag < 0) call check(nb*nodes,nstates_r,' nb*nodes,nstates_r')
  ...
  nstates = nstates_r
  if(rank==0)write(6,*) 'total number of read states is: ', nstates_r
  !allocate(phit(nx,ny,nz,nstates_r,nk_loc))
  !phit are the pseudo wfs
  if(rank==0) then
    allocate(phit_tot(nx,ny,nz,nstates,nk_loc))
    !for parallelization
  else
     !phit_tot is stored only on rank=0
  ...
  if (rank==0) then
    do i=1,nstates_r
      ...
      !if ( i .le. nocc) phi_bar_tot(:,:,:,i,1) = reshape(tmp,(/nx,ny,nz/))
      write(6,*) 'i, tmp', i, sum(abs(tmp))
      phit_tot(:,:,:,i,1) = reshape(tmp,(/nx,ny,nz/))!/norm
    enddo
  endif
end subroutine read_wf_tddft
