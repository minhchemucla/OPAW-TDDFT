subroutine get_dij_pert
    ...
    use paw_mod
    use tddft_mod, only : vxc_pert, vks_pert
    implicit none
...
!    if(k_flg) then 
    ...
    &     ucvol,charge,vks_pert,vxc_pert,xred)
    
        call transfer_dij_pert
      endif

      do ia=1,natom
        call bcast_r8(at(ia)%dij_pert,size(at(ia)%dij_pert),0)
      enddo
    ...
    subroutine set_rhoij
        ...
        do ia=1,natom
            ...
            do j=1,ndim
                do i=1,j
                    ind=ind+1
                    pawrhoij(ia)%rhoijp(ind,1)=at(ia)%rhoij_pert(i,j)
                enddo
            enddo
        ...
        do ia=1,natom
            ...
            do j=1,ndim
                do i=j,ndim
                   ...
                   at(ia)%dij_pert(i,j)=dd ! DN 
                   at(ia)%dij_pert(j,i)=dd ! DN
