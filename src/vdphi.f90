subroutine vd_phi(pin,hp,ik)
    ...
    integer :: ik
    complex*16 :: pin(nx,ny,nz), hp(nx,ny,nz)
...
end subroutine vd_phi

subroutine vd_pert_phi(pin,hp,ik)
    use main_mod
    use atom_mod
    use atom_mod, only : p=> pawinfo, at=> atominfo
    use tddft_mod, only : vks_pert
    implicit none
    integer :: ik
    complex*16 :: pin(nx,ny,nz), hp(nx,ny,nz)
            ...
            if(.not.at(ia)%edge) then
                do is=1,p(it)%mstates
                 hp(ixs:ixe,iys:iye,izs:ize)=&
                     hp(ixs:ixe,iys:iye,izs:ize)+&
                     at(ia)%local_p3d_c(:,:,:,is,ik)*&
                     sum(at(ia)%dij_pert(is,:)*ca(:))
                enddo
            else
                arr=matmul(at(ia)%dij_pert,ca)
                do iz=izs,ize;do iy=iys,iye;do ix=ixs,ixe
                    jx=mod(ix+nx-1,nx)+1
                    jy=mod(iy+ny-1,ny)+1
                    jz=mod(iz+nz-1,nz)+1
