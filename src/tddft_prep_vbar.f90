subroutine prep_vbar_sh
  use main_mod
  use tddft_mod, only : vbar_taylor_power, dt, phi_bar_tot
  use tddft_mod, only : na_node, nc_vbar
  use mat_module
  use mpi_lib_ours
  use paw_mod
  use atom_mod
  ...
  wtime = mpi_wtime()
  do nc_vbar=nodes,1,-1
    if(mod(natom,nc_vbar).eq.0) exit
  enddo
  if(mod(natom,nc_vbar).ne.0) stop 'there is a bug in calculating nc_vbar'
  na_node = natom/nc_vbar
  if(rank==0) write(6,*) 'na per node, nodes for vbar', na_node, nc_vbar
  ...
  do ia=1,natom
    ...
    call alloc_cv3d
  enddo
   do ia=rank*na_node+1,rank*na_node+na_node
    if (rank < nc_vbar) then
      !write(6,*) 'rank, ia', rank, ia
      it=atom_map(ia)
      ms = p(it)%mstates
      ...
      if(stat/=0) stop 'tmp_wf3d alloc problem in prep_vbar'
      allocate(vtmp_wf3d(nx,ny,nz,ms),stat=stat)
      if(stat/=0) stop 'vtmp_wf3d alloc problem in prep_vbar'

      allocate(mat_chk(ms,ms),stat=stat)
      if(stat/=0) stop 'mat_chk alloc problem in prep_vbar'
      allocate(mat_chk2(ms,ms),stat=stat)
      if(stat/=0) stop 'mat_chk2 alloc problem in prep_vbar'
      ...
      do i=1,ms
        at(ia)%local_cv_c(:,:,:,i) = vks*tmp_wf3d(:,:,:,i)
      enddo

      do i=1,ms
        do j=1,ms
          at(ia)%wvmat(i,j) = sum(conjg(tmp_wf3d(:,:,:,i))*at(ia)%local_cv_c(:,:,:,j))*dv
        enddo
      enddo

      call mat_inv(at(ia)%wvmat, at(ia)%avmat)

      at(ia)%local_cv1_c = 0d0
      do i=1,ms; do j=1,ms
        at(ia)%local_cv1_c(:,:,:,i) = at(ia)%local_cv1_c(:,:,:,i) + at(ia)%local_cv_c(:,:,:,j)*at(ia)%avmat(j,i)
      enddo;  enddo
      ...
      do i=1,ms
        do j=1,ms
          at(ia)%local_cv(:,:,:,i) = at(ia)%local_cv(:,:,:,i) + at(ia)%wvmat(j,i)*at(ia)%local_cv1_c(:,:,:,j)
        enddo
        mat_chk(i,:) = at(ia)%sinv(i,1)*conjg(at(ia)%wvmat(:,i))
      enddo
      if (sum(abs(at(ia)%local_cv - at(ia)%local_cv_c))*dv > 1e-11) then
        write(6,*) 'diff local_c = wc1 and local_cv_c greater than 1e-12:', &
              sum(abs(at(ia)%local_cv - at(ia)%local_cv_c))*dv
        stop
      else
      ...
      at(ia)%cvbarmat = mat_chk2
      at(ia)%cvmat = mat_chk
      do i=1,vbar_taylor_power
        at(ia)%cv_taylor = at(ia)%cv_taylor + mat_chk2
        at(ia)%cv_taylor_terms(:,:,i) = mat_chk2
        if(i.ne.vbar_taylor_power) mat_chk2 = 1/dble(i+1)*matmul(at(ia)%cvbarmat,mat_chk2)
      enddo
      deallocate(tmp_wf3d, vtmp_wf3d, mat_chk, mat_chk2)
      !write(6,*) 'test rank', rank
    endif
    !call bcast_c16(at(ia)%local_cv_c,size(at(ia)%local_cv_c),rank)
    !call bcast_c16(at(ia)%local_cv1_c,size(at(ia)%local_cv1_c),rank)
    !call bcast_c16(at(ia)%cv_taylor_terms,size(at(ia)%cv_taylor_terms),rank)
    !call bcast_c16(at(ia)%cv_taylor,size(at(ia)%cv_taylor),rank)
    !call bcast_c16(at(ia)%cvbarmat,size(at(ia)%cvbarmat),rank)
    !call bcast_c16(at(ia)%cvmat,size(at(ia)%cvmat),rank)
    !call bcast_c16(at(ia)%wvmat,size(at(ia)%wvmat),rank)
    !call bcast_c16(at(ia)%avmat,size(at(ia)%amat),rank)
  enddo
  !do ia=rank*na_node+1,rank*na_node+na_node
  wtime = mpi_wtime() - wtime
  if(rank==0) write(6,*) 'wtime prep_vbar', wtime
  wtime = mpi_wtime()
  do i=0,nc_vbar-1
    do ia=i*na_node+1,i*na_node+na_node
      !call bcast_c16(at(ia)%local_cv_c,size(at(ia)%local_cv_c),i)
      call bcast_c16(at(ia)%local_cv1_c,size(at(ia)%local_cv1_c),i)
      !call bcast_c16(at(ia)%cv_taylor_terms,size(at(ia)%cv_taylor_terms),i)
      call bcast_c16(at(ia)%cv_taylor,size(at(ia)%cv_taylor),i)
      !call bcast_c16(at(ia)%cvbarmat,size(at(ia)%cvbarmat),i)
      !call bcast_c16(at(ia)%cvmat,size(at(ia)%cvmat),i)
      !call bcast_c16(at(ia)%wvmat,size(at(ia)%wvmat),i)
      !call bcast_c16(at(ia)%avmat,size(at(ia)%amat),i)
    enddo
  enddo
  wtime = mpi_wtime() - wtime
  if(rank==0) write(6,*) 'wtime bcast', wtime

  !if(rank==0)call debug_vbar_sh
  call debug_vbar_sh_multi
contains
  subroutine alloc_cv3d
    implicit none

    allocate(at(ia)%local_cv(nx,ny,nz,ms))
    if(stat/=0) stop 'local_c alloc problem in prep_vbar'

    allocate(at(ia)%local_cv_c(nx,ny,nz,ms))
    if(stat/=0) stop 'local_cv_c alloc problem in prep_vbar'

    allocate(at(ia)%local_cv1_c(nx,ny,nz,ms))
    if(stat/=0) stop 'local_cv1_c alloc problem in prep_vbar'

    allocate(at(ia)%wvmat(ms,ms), at(ia)%avmat(ms,ms))  
    if(stat/=0) stop 'wvmat, amat'

    allocate(at(ia)%cv_taylor(ms, ms),stat=stat)
    if(stat/=0) stop 'cv_taylor alloc problem in prep_vbar'

    allocate(at(ia)%cv_taylor_terms(ms, ms, vbar_taylor_power),stat=stat)
    if(stat/=0) stop 'cv_taylor_terms alloc problem in prep_vbar'

    allocate(at(ia)%cvmat(ms, ms),stat=stat)
    if(stat/=0) stop 'cvmat alloc problem in prep_vbar'

    allocate(at(ia)%cvbarmat(ms, ms),stat=stat)
    if(stat/=0) stop 'cvbarmat alloc problem in prep_vbar'
  end subroutine alloc_cv3d

  subroutine debug_vbar_sh
    ...
    complex*16 :: wf_term(nx,ny,nz,vbar_taylor_power)
    complex*16 :: wf_term1(nx,ny,nz,vbar_taylor_power)
    ...
    complex*16 :: aaa(vbar_taylor_power)
    real*8  :: nrm0, nrm1, nrm2, nrm3, nrm4
    integer :: js, ks, iis, jjs, kks, ja, jt

    write(6,*) 'debug_vbar_sh'
    write(6,*) 'now checking s^-1t vs vbar=t+sum_i |p_i>y_i<c_i| various methods'
    ...
    do js=1,nocc
      ...
      tmp3 = 0d0
      tmp4 = 0d0
      call cvmat_phi(tmp,tmp5,ik)
      call cv_phi(tmp,tmp3,ik)
      ...
      do ia=1,natom
        ...
        if(stat/=0) stop 'y alloc problem in prep_vbar'


        !call vbar_proj2(ia, tmp, ca, ms)
        !call vbar_proj3(ia, tmp, ca3, ms)
        do is=1,ms
          ca(is) = sum(conjg(at(ia)%local_cv_c(:,:,:,is))*tmp)*dv
          ca3(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp)*dv
        enddo
          ...
          do is=1,ms
           ...
           ! at(ia)%local_p3d1_c(:,:,:,is,1)*y(is)*ca(is)
            tmp4(ixs:ixe,iys:iye,izs:ize)=tmp4(ixs:ixe,iys:iye,izs:ize)+&
              at(ia)%local_p3d1_c(:,:,:,is,1)*y(is)*sum(conjg(at(ia)%wvmat(:,is))*ca3(:))
            tmp5(ixs:ixe,iys:iye,izs:ize)=tmp5(ixs:ixe,iys:iye,izs:ize)+&
              at(ia)%local_p3d1_c(:,:,:,is,1)*sum(at(ia)%cvmat(is,:)*ca3(:))
          enddo
        else
          do iz=izs,ize;do iy=iys,iye;do ix=ixs,ixe
            jx=mod(ix+nx-1,nx)+1
  ...
  !------------------------------------
    ...
    !    if(stat/=0) stop 'mat_tmp alloc problem in prep_vbar'

    !    do is=1,p(it)%mstates
    !      do js=1,p(jt)%mstates
    !        mat_tmp(is,js) = sum(conjg(at(ia)%local_cv1_c(ixs:ixe,iys:iye,izs:ize,is))*&
  ...
  !------------------------------------
    ...
    do js=1,nocc
      ...
      call add_cvtaylor_phi(tmp,tmp5,ik)
      call add_cvtaylor_phi1(tmp,tmp16,ik)
      !call cv_taylor_loop(tmp,tmp21,ik)
      !tmp1 = tmp
      tmp1 = 0d0
      tmp17 = 0d0
      tmp18 = 0d0
      ...
      do ia=1,natom
        ...
        if(stat/=0) stop 'ca alloc problem in prep_vbar'
        allocate(ca2(ms),stat=stat)
        if(stat/=0) stop 'ca2 alloc problem in prep_vbar'

        do is=1,ms; ca(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp)*dv; enddo
        !write(6,*) 'ia, ca', ia, ca
        !write(6,*) 'ia', ia
        ...
        if(.not.at(ia)%edge) then
          ...
          do iis=1,ms
            tmp20(ixs:ixe,iys:iye,izs:ize) = tmp20(ixs:ixe,iys:iye,izs:ize)+&
               at(ia)%local_p3d1_c(:,:,:,iis,ik)*sum(at(ia)%cvbarmat(iis,:)*ca(:))
          enddo
          do is=1,ms; ca2(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp20)*dv; enddo
          !write(6,*) 'ia, ca2',ia, ca2
          do is=1,ms
            tmp1(ixs:ixe,iys:iye,izs:ize)=tmp1(ixs:ixe,iys:iye,izs:ize)+&
              at(ia)%local_p3d1_c(:,:,:,is,1)*sum(at(ia)%cv_taylor_terms(is,:,1)*ca(:))
            tmp19(ixs:ixe,iys:iye,izs:ize)=tmp19(ixs:ixe,iys:iye,izs:ize)+&
              at(ia)%local_p3d1_c(:,:,:,is,ik)*sum(at(ia)%cvbarmat(is,:)*ca2(:))/2d0
      ...
      do ia=1,natom
        ...
        allocate(ca(ms),stat=stat)
        if(stat/=0) stop 'ca, y alloc problem in prep_vbar'
        do is=1,ms; ca(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp17)*dv; enddo
    ...
    !      write(6,*) 'test1', y
          if(.not.at(ia)%edge) then
            !write(6,*) 'i', i
            do is=1,ms
              !ca(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp)*dv
              ca2(is) = sum(conjg(at(ia)%local_cv_c(:,:,:,is))*tmp13)*dv
              ca3(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp14)*dv
              ca4(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp)*dv
            enddo
            call proj1(ia, tmp10, ca, ms, ik)
            !write(6,*) 'ia,i ca3', ia, i, ca3
            do is=1,ms
              tmp3(ixs:ixe,iys:iye,izs:ize)=tmp3(ixs:ixe,iys:iye,izs:ize)+&
                at(ia)%local_p3d1_c(:,:,:,is,1)*sum(at(ia)%cvbarmat(is,:)*ca3(:))/dble(i)
              ...
              !  at(ia)%local_p3d1_c(:,:,:,is,1)*sum(at(ia)%cv_taylor_terms(is,:,i)*ca4(:))
            enddo
            ...
            if(i .eq. 1) then 
              ...
              do is=1,ms
                ca3(is) = sum(conjg(at(ia)%local_cv1_c(:,:,:,is))*tmp3)*dv/dble(i)
              enddo
      ...
      wf1 = reshape(tmp1, (/nn/)) ! same as add_cv_taylor
      wf2 = reshape(tmp, (/nn/))  !intput
      wf3 = reshape(tmp4, (/nn/)) !e^sum = 1 + sum + ...
      wf4 = reshape(tmp5, (/nn/)) ! add_cv_taylor
      ...
      !nrm4 = sum(conjg(wf5)*wf5)*dv
      aa = sum(abs(wf5-wf1))*dv !exact taylor - add_cv_taylor
    ...
    do js=1,nocc
      ...
      do i=1,vbar_taylor_power
        ...
        tmp1 = tmp3
      enddo
      !call prop_vbar_split_sh(tmp, tmp4, ik)  
    ...
    enddo
  end subroutine debug_vbar_sh

  subroutine debug_vbar_sh_multi
  ...
  !------------------------------------
    ...
    if (rank==0) write(6,*) 'now checking e^s^-1t vs split step'
    call sync_mpi
    ...
    do js=1,nb
      ...
      do i=1,vbar_taylor_power
        !call st_phi(-ci*dt/dble(i)*tmp1, tmp3, ik)
        call sn_phi(-ci*dt/dble(i)*vks*tmp1, tmp3, 1,-1d0)
        !call t_phi(-ci*dt/dble(i)*tmp1, tmp3, ik)
        !call sum_inv_T_phi(-ci*dt/dble(i)*tmp1, tmp3, ik)
        !aa = aa/dble(i)
        tmp2 = tmp2 +tmp3
      ...
      enddo
      w2time = mpi_wtime()
      call prop_vbar_split_sh(tmp, tmp4, ik, dt)  
    ...
    enddo
    call sync_mpi
    write(6,*) 'rank, walltime apply vbar', rank, w1time
  end subroutine debug_vbar_sh_multi
end subroutine prep_vbar_sh
